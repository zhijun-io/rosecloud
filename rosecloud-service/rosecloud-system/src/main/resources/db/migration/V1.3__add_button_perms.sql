...
-- endpoints (notice publish, audit log, login log). Each becomes a
...
INSERT INTO sys_menu (id, parent_id, name, type, path, component, perms, icon, sort, status, visible, deleted) VALUES
(44, 2,  '审计日志', 1, 'audit',     'system/audit/index',  'system:audit:list', 'security', 11, 1, 1, 0),
(45, 44, '审计查看', 2, NULL,        NULL,                   'system:audit:list', NULL,       1, 1, 1, 0)
ON DUPLICATE KEY UPDATE name = VALUES(name);

-- Login log (under 系统管理): menu + view button.
INSERT INTO sys_menu (id, parent_id, name, type, path, component, perms, icon, sort, status, visible, deleted) VALUES
(46, 2,  '登录日志', 1, 'login-log', 'system/login-log/index', 'system:loginlog:list', 'log', 12, 1, 1, 0),
(47, 46, '日志查看', 2, NULL,        NULL,                      'system:loginlog:list', NULL, 1, 1, 1, 0)
ON DUPLICATE KEY UPDATE name = VALUES(name);
...
(47, 1, 44),   -- platform-admin: audit menu
(48, 1, 45),   -- platform-admin: audit view
(49, 1, 46),   -- platform-admin: login log menu
(50, 1, 47)    -- platform-admin: login log view
