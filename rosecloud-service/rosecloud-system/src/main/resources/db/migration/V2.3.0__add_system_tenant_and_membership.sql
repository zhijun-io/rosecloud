...
-- 默认系统租户：全零 UUID，平台管理员所属，保留不可经普通流程变更
INSERT INTO sys_tenant (id, name, status, create_time, update_time, deleted)
SELECT '00000000-0000-0000-0000-000000000000', '系统租户', 1, NOW(), NOW(), 0
WHERE NOT EXISTS (
    SELECT 1 FROM sys_tenant WHERE id = '00000000-0000-0000-0000-000000000000'
);
...
WHERE u.deleted = 0
  AND NOT EXISTS (
    SELECT 1 FROM sys_user_tenant t WHERE t.user_id = u.id AND t.tenant_id = COALESCE(u.tenant_id, '00000000-0000-0000-0000-000000000000')
  );
