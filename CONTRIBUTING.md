...
- **启动类**：业务服务把 Feign 扫描单独放到 `*FeignConfiguration`，用 `@EnableFeignClients(basePackages = "io.rosecloud.api")` 显式扫描 `rosecloud-api` 的 Feign 契约；单体通过 `monolith` profile 激活本地 adapter，组件扫描排除这些 `*FeignConfiguration`；网关只用 `@SpringBootApplication`（WebFlux，不开 Feign）
...
- **网关路由**：动态路由由 `rosecloud-gateway` 的 `MetadataRouteDefinitionLocator` 按服务发现元数据 `gateway.path` 自动生成（`lb://` + Path 断言，路由 id `dyn-{serviceId}`，支持逗号分隔多路径）——新服务只需在自身 `spring.cloud.nacos.discovery.metadata.gateway.path` 声明路径即接入，无需改网关；`RouteRefreshScheduler` 启动时及每 30s 发布 `RefreshRoutesEvent` 刷新
...
- **子节点归属判据**：看配置项"离开父节点是否还成立"——成立则与父平级，不成立则下沉为子节点。例：吊销离了 jwt 无意义，下沉 `jwt.revocation`；内部密钥离了 jwt 照常工作，平级 `security.internal`
- **类型化**：每个 `rosecloud.*` 域必须有且仅有一个 `@ConfigurationProperties` 类承载字段。禁止裸 `@Value("${rosecloud.*}")`，禁止仅靠 `@ConditionalOnProperty` 而无对应 properties 类的域
- **环境变量对齐**：环境变量名必须是 prefix 的下划线大写形式，走 Spring relaxed binding 直连，如 `rosecloud.security.jwt.secret` ↔ `ROSECLOUD_SECURITY_JWT_SECRET`。不靠 yaml `${ENV}` 占位符"桥接"——占位符仅用于默认值兜底（`${ENV:默认值}`），且 prefix 本身必须与 env 对齐，避免"改了 env 不生效、靠默认值掩盖"的隐患
- **部署形态不进 prefix**：单体/微服务/网关的差异用 `@Profile` / `@ConditionalOnWebApplication` / `@ConditionalOnProperty` 控制，不把 `monolith` / `gateway` 写进 prefix。同一概念（如免鉴权白名单）用统一 prefix + 统一 properties 类共享，不按部署形态复制多个类
- **后端类型词汇统一**：`redis`（跨进程共享）/ `memory`（单进程内存）/ `local`（本地磁盘/文件），不使用 `in-memory`、`mem` 等变体
- **边界**：Nacos discovery metadata 的 key 不带 `rosecloud.` 前缀（它不是应用配置属性，如 `gateway.path`）；标准 Spring 配置（`spring.*`）不归入 `rosecloud.*`
- **目标结构**：

```yaml
rosecloud:
  cache: {enabled, type}              # 技术能力层
  lock: {enabled, type}
  sequence: {enabled, type}
  storage: {enabled, type, base-dir}
  datasource: {dynamic: {enabled}}
  tenant: {enabled, type, ignore-tables}
  audit: {enabled}
  security:                           # 安全机制层
    jwt:                              # 签发/校验/生命周期
      secret, allow-dev-secret, issuer, access-ttl, refresh-ttl
      revocation: {type}              # 从属 jwt：吊销存储
    oauth2: {enabled}                 # 资源服务器，与 jwt 平级
    internal: {api-key}               # 服务间 /internal 密钥，与 jwt 平级
    public-paths: [list]              # 免鉴权路径，与 jwt 平级
  auth:                               # 认证业务层
    login-protection: {enabled, max-attempts, ...}
    mfa: {…}
  notice: {publish-check-ms, ...}     # 服务业务层
```
