...
public class DeptServiceImpl implements DeptService {
    ...
    public Long create(DeptRequest request) {
        return deptRepository.insert(toDept(null, request));
    }

    @AuditLog(action = "dept-update", description = "修改部门")
    @Override
    public void update(Long id, DeptRequest request) {
        deptRepository.findById(id).orElseThrow(() -> new BizException(SystemErrorCode.DEPT_NOT_FOUND));
        deptRepository.update(toDept(id, request));
    }
