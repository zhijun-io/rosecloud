...
- 平台管理员归属**系统租户**（保留租户，id 为 `ROOT`）；系统租户上下文为平台视角，行级隔离豁免，可见全部租户数据
...
- 受控租户切换的上下文与成员查询基础已具备；正式的 `switch-tenant` 对外接口仍需按 `auth` / `system` 职责边界完成落地
...
- 平台管理员归属系统租户（id 为 `ROOT`），登录时活动租户为系统租户，无需显式租户标识
