...
目标：先把后端闭环跑通，不做前端页面。优先完成“租户申请 -> 审核 -> 开通 -> 初始化 -> 通知 -> 审计”这条主链路。
...
目标：把租户申请、审核、开通、初始化、通知和审计串成一条可验收的后端闭环。

### 2.1 租户申请与审核

#### 模块边界

- `system` 负责租户申请、审核、开通、状态流转和初始化编排。
- `notice` 负责审核结果、开通结果和后续触达通知。
- `auth` 负责登录侧的租户语义、会话和上下文。
- `audit` 负责关键动作的审计记录与追踪。

#### 状态机

- 申请提交后进入 `PENDING`
- 审核通过后进入开通流程，完成后进入 `ENABLED`
- 审核拒绝后进入 `DISABLED`
- 管理员停用后进入 `DISABLED`
- 到期后进入 `EXPIRED`
...
- `system_setting` 负责平台级设置，字段为 `key / value / updated_at / updated_by`
- `user_setting` 负责用户级设置，字段为 `user_id / key / value / updated_at / updated_by`
